...
- `evals/run_redteam.py` maps every attack class to named unit tests, loads the suite in-process, and fails on unmapped tests (117 classes, 157 tests). `run_adversarial.py` measures adapter calls and permits issued/consumed. `run_state_fuzz.py` advances a shared clock. The model checker models two permits, in-flight submission, expiry, voiding, halt and resume, and settlement lag, and reports the counterexamples without the permit-window rule and without the consumed-permit ledger check.
