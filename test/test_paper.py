...
from support import AUTH, NOW, attest_pair, trust
...
class PaperVenueTests(unittest.TestCase):
    def setUp(self):
        ...
        self.venue = PaperTradingVenue(
            name="paper-dex", prices_usd={"MEME": Decimal("0.50")}, balances={"USDC": Decimal("1000")}
        )
        self.trust = trust()
        self.runtime = FAARRuntime(self.store, {"paper-dex": self.venue}, self.trust, allow_test_time_override=True)
