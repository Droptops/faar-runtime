...
| `transition(id, expected, new, *, reason_codes, effect_id, release_usage, commit_usage, ambiguity_until, filled_amount_usd)` | Compare-and-set on the stored state; only transitions in `_ALLOWED_TRANSITIONS`; `release_usage`/`commit_usage` update the reservation in the same txn; setting `effect_id` enforces uniqueness per `(venue, effect_id)` (`EffectConflict`) in that same txn, which is how an unfilled cancel claims its order identity atomically with its release; `filled_amount_usd` records the last accepted cumulative fill; refuses an oversized reason-code list (`EvidenceRecordTooLarge`). **LP** for terminalization. | I-3, I-11, I-35, I-38 |
| `begin_submission(id, expected, *, max_attempts, now)` | In one txn, checks the immutable velocity limit snapshot, CASes into SUBMITTED, increments the durable counter and appends one immutable `submission_attempts` row; resets `ambiguity_until`; refuses past either the retry limit or `max_actions_per_window`. A pending reservation occupies one provisional velocity slot and is atomically replaced by the first attempt, while every retry consumes another. **LP** for "an attempt began". | I-5, I-13 |
| `get(id)` | Reads the row; unknown ids raise `UnknownIntent`. | |
| `intent_guard(id, wait_seconds)` | Durable per-intent lease recording owner token (the store instance), host and pid: at most one worker inside an intent's state machine across processes; the same instance may re-acquire its own lease from any thread; a dead worker's lease is never taken over automatically (`IntentBusy`); `clear_stale_intent_lease` needs the exact owner token and refuses a live local owner unless forced; a busy datastore raises `StoreUnavailable`, never a bare driver error, and a failed release waits a bounded time (about 5 s). | I-6, OPERATIONS §2 |

## 3. Budget and risk ledger

| Method | Guarantee | Notes |
|---|---|---|
| `reserve_usage(intent, grant, risk, now)` | One txn that checks and inserts: risk-state version uniqueness (exact version in both ledgers) and monotonicity across both risk ledgers, trailing-window turnover over HELD+COMMITTED rows of every version of the grant id owned by the principal (plus rows migrated without a principal), sliding-window velocity over immutable attempt rows plus pending reservations, scope exposure caps; inserts the HELD reservation, immutable velocity-limit snapshot and risk claim together. **LP** for aggregate limits. | I-13, I-14, I-37 |
