...
| RT-80 | `FINALIZED` and the usage commit were two autocommit statements; a worker killed between them left budget HELD on a FINALIZED intent forever (found by the new crash-injection eval, which kills a worker before every store call) | Low | `transition(..., commit_usage=True)` finalizes and commits in one transaction; a replay repairs rows written by older versions; `make crash` runs 309 crash points across 9 scenarios in `make check`, including contradictory-settlement and cumulative-fill-regression stops |
...
| RT-88 | The evidence bounds introduced for RT-83 applied to the quorum's own aggregate record too: members each inside the bound summed past it, the aggregate failed its constructor on every reconcile, and an existing effect could never be finalized (budget held forever) | Medium | The aggregate falls back to a compact record carrying each member's evidence hash; effect ids in aggregate facts are truncated; `effect_id` and `verified_request_hash` are length-bounded at construction |
| RT-89 | The crash-injection predicate accepted two venue calls in every scenario and any settlement-derived STOP with held budget, so a duplicate submission absorbed by the idempotent mock venue, or a change that made every scenario stop, would have passed | Assurance | Per-scenario call ceilings and acceptable outcome sets; resubmission after an effect, more than one consumed permit, and unexpected stop reasons are violations |
| RT-90 | A `Decimal` subclass with overridden comparison survived the bounded parser and finalized an intent whose settled amount was ten times the authorization; an exposure cap tightened below current turnover also blocked zero-notional cancels; an unreadable stored cap raised out of `process()` | Low | `parse_bounded_decimal` returns a plain copy; zero-notional actions bypass the cap; unreadable caps fail closed with `EXPOSURE_CAP_UNREADABLE` |

| RT-91 | Lifecycle changes, fences and consumptions committed to the datastore before the anchor mark was raised; a crash or transient anchor failure in between left the anchor behind, and a later restore of a pre-revoke backup resurrected the revoked grant undetected | High | The mark is raised inside the transaction; provisioning, re-activation, issuance and consumption roll back on anchor failure; pause, revoke, halt and restore-recovery commit regardless (stopping is the safe direction) and report the anchor failure |
| RT-92 | `database is locked` past the busy timeout escaped `process()` as a raw driver error and left the live worker's own lease behind, which it could not re-acquire (`INTENT_BUSY` for ever) | Medium | An owner may re-acquire its own lease; release retries with backoff; `StoreUnavailable` / busy errors become the result `STORE_UNAVAILABLE` with the state unchanged |
| RT-93 | The file anchor took an untimed `flock`, in its constructor too: a stalled holder hung every worker inside `process()` and hung `faar halt` before it reached the store | Medium | Bounded lock wait (`ANCHOR_LOCK_TIMEOUT_SECONDS`) raising `AnchorUnavailable`; no lock on open when the file exists; `halt` commits even when the anchor is unreachable |
| RT-94 | The durable lease carried no liveness identity, so the runbook's "confirm the owner is dead" step could not be performed; clearing a live owner's lease ended with an unrecorded effect and stranded budget | Medium | Leases record host and pid; `clear-lease` refuses a live local owner unless `--force`; cross-host liveness stays the operator's step |
| RT-95 | Whole-second stamps with a strict lower bound made every trailing window (velocity, turnover, exposure caps) up to one second shorter than configured: two actions 100 ms apart passed a one-action-per-second limit | Medium | Inclusive lower bound: truncation can only widen a window |
| RT-96 | Time-valued grant limits and verifier skews were unbounded (`OverflowError` out of `process()`); a naive runtime clock raised `TypeError` after registration; the permit gateway's default skew (2 s) was below the grant default (5 s); `checkpoint` ignored a busy result and the runbook copied a file missing WAL frames; reconciliation accepted any provisioned grant, so a foreign grant's skew could shorten the ambiguity window | Low | Limits and skews bounded to a leap year; clocks validated at construction; gateway default 5 s; `checkpoint()` returns False and the CLI exits 2; in-flight intents accept only their own grant (`GRANT_NOT_BOUND_TO_INTENT`) |

| RT-97 | The anchor binding recorded only presence: a fresh or replaced anchor file at the configured path (unmounted volume, wrong host) opened cleanly and every REGRESSED grant reported ACTIVE, so a restored database resurrected a revoked grant and re-consumed a consumed permit | High | The first anchored open binds an anchor identity in both the datastore and the anchor; a different or identity-less anchor is `AnchorMismatch` at open |
...
| RT-115 | An unfilled `CANCELLED` record carrying another intent's order identity at the same venue released the budget as absence | Low | The order identity is claimed in the same transaction as the release (per-venue unique index); another owner is `EFFECT_ID_ALREADY_CLAIMED`, budget held |
...
| RT-117 | The RT-115 ownership check was a lookup outside the release transaction: a concurrent reconciliation could bind the same order id between the check and the release, and the cancel then released the budget anyway | Medium | The identity is claimed inside the release transaction (`transition(..., effect_id=...)` under `ux_effect_id_per_venue`); `EffectConflict` becomes `EFFECT_ID_ALREADY_CLAIMED` with the budget held |
| RT-118 | Windows keyed by grant id alone (RT-102) let another principal's version of the same grant id exhaust a tenant's turnover and velocity | Medium | Windows are keyed by principal and grant id; rows migrated without a principal count for every principal of the grant id |
| RT-119 | A stop that committed while the anchor write failed left the anchor permanently behind: re-running the same revoke was a silent no-op, nothing reported the gap, and a later pre-revoke restore resurrected the grant and executed an effect | High | Re-running a stop raises the mark; every anchored open raises marks behind rows; `list-grants` reports `anchor_behind`; the error is `AnchorUnavailableAfterCommit` and the CLI prints `committed: true` |
| RT-120 | Pause, revoke and `revoke_after_restore` rolled back when the anchor could not be read (only `halt` committed), contradicting the documented stop-direction rule; the CLI could not distinguish "committed, anchor not updated" from "not committed" | Medium | An unreadable anchor never blocks a stop (regression only matters for loosening); `revoke_after_restore` proceeds from the row; `committed: true/false` in the CLI error |
| RT-121 | The durable lease owner token included the thread ident, so a pooled worker whose release failed could not re-acquire its own lease from another thread and `clear-lease` refused the live pid | Low | The owner token is the store instance (threads are serialized in process); the release waits at most about five seconds instead of attempts x busy timeout |
| RT-122 | The born-with-head watermark was set to the next evidence id at first open by this build, so chains written by an earlier head-writing 0.4 build were classified legacy and a truncated one could be rebuilt over | Low | The watermark is the first chain that starts at `intent_registered` (introduced together with signed heads); such chains are never legacy |
| RT-123 | A signed authority decision with thousands of reason codes made the terminal transition raise `EvidenceRecordTooLarge` before anything was persisted; the intent stayed PROPOSED with no code | Low | `AuthorityDecision` bounds its reason codes at construction (64 codes of at most 256 characters) |
| RT-124 | `verify_task_outcome` raised `ValueError` when overlapping criteria copied a within-bound settlement evidence past the 64 KiB budget | Low | `OUTCOME_EVALUATION_UNBOUNDED`: MET becomes UNKNOWN, NOT_MET keeps its failures without the evaluation record |
| RT-125 | A market order carrying a decorative `limit_price` satisfied the executor-side bound (RT-103) | Low | A limit price counts only for an order declared `order_type: limit` |
...
The reference models open orders, monotone cumulative partial fills, filled and unfilled cancellation, and fail-closed contradictions. It deliberately relies on a venue reporting `CANCELLED` only once no later fill is possible. A live adapter must document and test that terminality guarantee; the mock cannot prove it for an external venue.
