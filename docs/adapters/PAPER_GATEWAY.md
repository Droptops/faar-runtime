...
`ExecutionPermitVerifier.consume` runs inside `PaperVenueService.submit` before
any balance change. The adapter process does not consume the permit. A rejected
permit is `PERMIT_REJECTED:<codes>` and creates no order.
...
| `REJECTED` / `CANCELLED` | `NONE` | yes | the venue will not fill that identity later |
| `PENDING` | `UNKNOWN` | no | a GTC order may still fill (B3) |
...
A fill's `effect_id` is `pg_` + SHA-256(`venue`, `client_order_id`, canonical
fill)[:24]. It is assigned only when the book fills and does not change. A
cancel is a separate intent with its own `effect_id` and `amount_usd=None`.

## finality definition

Paper fills have no clearing delay. A fill is immediately `FINALIZED`. There
is no `CONFIRMED`-then-later-final path. Gate 4.9 for this venue.

## partial fills

Not modelled. A marketable order fills completely at the book's fill price or
not at all. An unmarketable IOC/FOK (default `time_in_force`) is
`LIMIT_PRICE_EXCEEDED` with no resting order. This does not close runtime
gate 4.4.
...
- `FILLED` → `ORDER_ALREADY_FILLED`. Balances are not unwound.
- already `CANCELLED` → cancel is idempotent (state stays `CANCELLED`).
- missing → `ORDER_NOT_FOUND`.

A cancelled order's original intent reconciles as authoritative `NONE`.
...
a production-safety claim. It turns the paper-gateway rows for gates 4.2, 4.9,
6.7 and residual R-12 into in-repo tests. A real venue still needs its own
review document and the remaining DEPLOYMENT rows.
